...
from app.models.schemas import SubmitAssignmentRequest, GradeSubmissionRequest, AssignmentCreateRequest, AssignmentUpdateRequest
...
def grade_submission(req: GradeSubmissionRequest, current_user: dict = Depends(role_required(["Teacher"]))):
    ...
    # Verify teacher owns the course of this submission
...
def delete_assignment(assignment_id: int, current_user: dict = Depends(role_required(["Teacher"]))):
    ...
    # No check for existing submissions – the stored procedure handles recalculation
